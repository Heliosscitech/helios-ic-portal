  ...
  return (
    ...
    >
      <div className="absolute top-0 right-0 p-3">
        <div className={cn('px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border', getTypeStyles(contact.type))}>
          {TYPE_LABEL[contact.type]}
        </div>
      </div>

      <div className="flex items-center gap-4 mb-6">
        <div className="w-14 h-14 rounded-full bg-linear-to-br from-[#E8F0F7] to-[#c8d8e8] flex items-center justify-center text-lg font-bold text-[#2E5A7C] shrink-0">
          {getInitials(contact.name)}
        </div>
        <div className="min-w-0">
          <h3 className="text-[15px] font-bold text-text group-hover:text-[#0C447C] transition-colors">{contact.name}</h3>
          <p className="text-[12px] text-text-3 font-medium">{contact.title}</p>
          {contact.company && (
            <p className="text-[12px] text-[#0C447C] font-bold mt-0.5 flex items-center gap-1 truncate">
      ...
      <div className="space-y-2.5 pt-4 border-t border-dashed border-border">
        {contact.email ? (
          <span
            ...
            className="flex items-center gap-3 text-[12px] text-text-2 hover:text-[#0C447C] transition-colors"
          >
            <Mail size={14} className="text-text-3 shrink-0" />
            <a href={`mailto:${contact.email}`} className="truncate">{contact.email}</a>
          </span>
        ) : (
          <span className="flex items-center gap-3 text-[12px] text-text-3">
        ...
        {contact.phone ? (
          <span
            onClick={(e) => e.stopPropagation()}
            className="flex items-center gap-3 text-[12px] text-text-2 font-mono"
          >
            <Phone size={14} className="text-text-3 shrink-0" />
            <a href={`tel:${contact.phone}`}>{contact.phone}</a>
          </span>
        ) : (
          <span className="flex items-center gap-3 text-[12px] text-text-3 font-mono">
      ...
      {contact.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-4">
          {contact.tags.map((tag) => (
            <span
              key={tag}
              className="text-[10px] bg-surface-2 text-text-3 px-2 py-1 rounded-md font-bold flex items-center gap-1"
            >
              <Tag size={8} /> {tag}
            </span>
          ))}
